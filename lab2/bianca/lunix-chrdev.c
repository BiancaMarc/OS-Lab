...
static int lunix_chrdev_state_update(struct lunix_chrdev_state_struct *state)
{
	...
	if(lunix_chrdev_state_needs_refresh(state)){
		...
		if(state->type == 0)
			modified = lookup_voltage[val1];
		else if(state->type == 1)
			modified = lookup_temperature[val1];
		else if(state->type == 2)
			modified = lookup_light[val1];
		else {
			debug("error on sensor type, did not get lookup table value\n");
			up(&state->lock);
			return -1;
		}
		/*sprintf returns the total number of characters writted, excluding
		  the null character at the end. On failure, a negative number is returned*/
		state->buf_lim = sprintf(state->buf_data, "%ld.%03ld\n", modified/1000, modified%1000);
		state->buf_timestamp=now;
		debug("%ld\n",state->buf_lim);
		return state->buf_lim;
	}
	else {
		debug("State needs refreshment, trying again\n");
		return -EAGAIN;
	}
}
...
{
	debug("I am releasing memory now\n");
...
static ssize_t lunix_chrdev_read(struct file *filp, char __user *usrbuf, size_t cnt, loff_t *f_pos)
{
	 ...
	 */
	if (*f_pos == 0) {
		debug("Just entered read-> if f_pos==0\n");
		while (lunix_chrdev_state_update(state) == -EAGAIN) {
			...
			if(wait_event_interruptible(sensor->wq,lunix_chrdev_state_needs_refresh(state))){
				debug("Something is wrong: process didn't go to sleep\n");
				return -ERESTARTSYS;
			}
			if(down_interruptible(&state->lock))
				return -ERESTARTSYS;
...
out:
	/* Unlock? */
	up(&state->lock);
	debug("Exiting read with ret value %zd\n", ret);
	return ret;
}
